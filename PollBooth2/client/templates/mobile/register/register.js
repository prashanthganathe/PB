Template.register.events({
    ...
    'click #btnregister': function(event) {     
        let user = {
            ...
            fname : $('#fname').val(),
            profile : { 'firstname': $('#fname').val(), 'lastname': '' }
        };       
       /* Accounts.createUser(user, (error) => {
        ...
        function getWelcomeObject(name, email, verifyurl) {
            console.log('inside getWelcomeObject');
            var emailData = {
            ...
            var welcomobj = {};           
            welcomobj.name = name;
            welcomobj.title = "Welcome to PollBooth";
            welcomobj.from = "noreply@wapp.com";
            welcomobj.to = email;
            welcomobj.description = "Welcome Email";
            welcomobj.templatename = "welcome.html";
            welcomobj.attachment = [];
            welcomobj.jsonvalues = emailData;
            welcomobj.type = "Welcome";
            welcomobj.starttime = new Date();
            welcomobj.expirytime = new Date().addHours(24);
            welcomobj.status = "Pending";
            welcomobj.retrycount = 3;
            welcomobj.failurereason = "";
            welcomobj.createdat = new Date();
            console.log(welcomobj);
            return welcomobj;
        }

        function Loginwithregister(email, password) {
            console.log('user' + 'Loginwithregister');
            Meteor.loginWithPassword(email, password, function(err, result) {
                if (err) {
                    console.log('else of loginWithPassword');
                } else {
                    console.log('else of loginWithPassword');
                }
            });
        ...
        Meteor.call('createUserWithRole', user,  function(error, result) {
            if (error) {
                Bert.alert('Some problem with Registration, please retry with other credentials.', 'danger', 'fixed-top', 'fa-frown-o');
                console.log(error);
            } else {

                var verifyurl = result;
                console.log('success' + verifyurl);
                Bert.alert('Successfully Registered. Please check your email to activate and proceed.', 'success', 'fixed-top', 'fa-frown-o');
                var emailobj = getWelcomeObject(user.fname,  user.email, verifyurl);
                console.log(emailobj);
                var welcomemail = Emailcrons.insert(emailobj);
                FlowRouter.go('/thankyou');               
            }
        });
    ...
    'click #google-login': function(event) {
        Meteor.loginWithGoogle({ requestPermissions: ['email', 'profile'] }, function(err) {
            if (err) {
                ...
                FlowRouter.go('/presenterdashboard');
            }
        });
    },
